...
export default function PlaylistListItem({ playlist }: PlaylistListItemProps) {
  const cover = useGetCoverArt(playlist.coverArt, { size: 200 });

  return (
    <Link href={`/playlists/${playlist.id}`} className="mr-6" asChild>
      <Pressable>
        {({ pressed }) => (
          <Animated.View
            className="transition duration-100 gap-y-2 w-32"
            style={{
