...
const CheckboxIcon = React.forwardRef<
  ...
  ICheckboxIconProps
>(function CheckboxIcon({ className, ...allProps }, ref) {
  const { size, ...props } = allProps as ICheckboxIconProps & {
    size?: number;
    height?: number;
    width?: number;
  };
  if (typeof size === "number") {
    return (
      <UICheckbox.Icon
        ref={ref}
        {...(props as ICheckboxIconProps)}
        className={checkboxIconStyle({ class: className })}
        size={size}
      />
    );
  } else if (
    ((props as { height?: number }).height !== undefined ||
      (props as { width?: number }).width !== undefined) &&
    size === undefined
  ) {
    return (
      <UICheckbox.Icon
