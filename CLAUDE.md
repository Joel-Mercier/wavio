...
`react-native-audio-pro` is the audio engine. `services/playbackService.ts` is the registered background service; the queue store drives it. Offline downloads go through `services/offlineDownloadService.ts` + `hooks/useOfflineDownloads.ts` using `expo-file-system`.
...
- `patch-package` runs on `postinstall`; check `patches/` before upgrading patched deps.
