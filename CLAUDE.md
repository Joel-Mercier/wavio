...
- Car work only runs **while a car host is connected**. This covers the browse-tree build, its cover mirror and the 1 s position pulse.
  ...
  - RN pauses JS timers whenever no Activity is resumed, which is the normal state in a car. While a car is connected, `CarTimerHold.kt` therefore holds a headless JS task. JS registers it as `WavioCarSession` in `session.ts`, and it must be registered before `notifyReady()`.
  - Without that task, any `setTimeout` in car code (the `mapInChunks` yields, debounces, fetch deadlines) hangs until the phone UI opens.
  - The task alone is not enough on every phone. RN timers fire from Choreographer frames, and MIUI delivers none to an idle background app (verified).
  - So car-session waits use `CarAutoBridge.delay(ms)`, which is driven by a native `Handler` message. Never use a bare `setTimeout` / `setInterval` in car code.
