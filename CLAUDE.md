...
- `EXPO_PUBLIC_LASTFM_API_KEY`, `EXPO_PUBLIC_LASTFM_API_SECRET` — Wavio's own Last.fm API account, all three environments (`sensitive`, not `secret`, so `eas build --local` can read them). A build without them can't connect; `hasLastFmCredentials()` (`services/lastFm/config.ts`) gates the settings screen so that fails legibly instead of as an opaque error 10. Only the *write* path needs the secret — every Last.fm read method (`user.*`, `artist.getSimilar`, `track.getInfo`) takes the api key alone and is never signed.

  **Shipping the shared secret is a deliberate, reconsidered decision, not an oversight.** Last.fm has no OAuth2 and signs every write with `md5(params + shared secret)` computed client-side, so a client that scrobbles directly *must* carry it; the only alternatives are a Wavio-operated signing proxy (no such infrastructure, and it would see every user's listening history) or making every user register their own API account. Be clear-eyed about what it costs, because this is **not** the same risk as the Taddy key above — that one is a quota credential, this one is an *identity* credential:
  - Whoever extracts it can sign requests **as Wavio**, including standing up a Last.fm consent screen branded "Wavio" and harvesting session keys (scrobble / love / `track.removeScrobble` on those accounts — not account takeover, but a phishing primitive).
  - If the key is abused, Last.fm suspends it (error 26) and Last.fm breaks for **every Wavio user at once**, recoverable only by shipping a new build with a new key.
  - `EXPO_PUBLIC_` inlines it into the JS bundle, so `strings` on the APK finds it, and the bundle also reaches Sentry as sourcemaps. A native `BuildConfig` field would need `apktool` instead — a speed bump, not a boundary, which is why it isn't worth doing. There is no OTA exposure (no `expo-updates`, no `updates` config in `app.json`).
  - Nothing in Last.fm's ToS requires the secret to be confidential or forbids one account serving a distributed app, so this is a risk decision rather than a compliance one.
...
**Background timers.** JS `setTimeout` / `setInterval` stop within seconds of the app leaving the foreground and never fire again until the UI reopens — with audio playing, with or without a car (measured on MIUI; RN pauses timers when the Activity pauses). Anything that must keep running during background playback (network probes and recovery polls, prefetch / download / scrobble / replay backoffs, the play-queue sync debounce, the throttled MMKV and React Query persistence flushes, the jukebox status poll and the remote-output lock-screen pulse) uses `services/backgroundTimer.ts` (`setBackgroundTimeout` / `setBackgroundInterval` / `clearBackgroundTimer` / `backgroundSleep`), driven by a main-thread `Handler` message in `modules/car-auto` and falling back to JS timers on iOS and in tests. Keep plain JS timers for UI-only work (debounced inputs, animations, progress tickers) and for polls that should stop in the background — the reachable-state heartbeat in `services/network.ts` is one on purpose, so an idle backgrounded app never pings the server every 30 s. The unreachable-server recovery poll is on a background timer but gated by `services/recoveryPollGate.ts` (wired from `index.js`): it only runs while the app isn't backgrounded, audio is playing, or a car is connected.
...
**Remote outputs** (Subsonic jukebox, UPnP/DLNA renderers, Chromecast) are all `RemoteTarget`s registered from their own service module (`services/jukebox.ts`, `services/upnp.ts`, `services/cast.ts`) via `services/playback/remoteTarget.ts`; `services/playback/targets.ts` imports the three so registration happens even on a headless (Android Auto) boot. `services/player.ts` checks `activeRemoteTarget()` at every transport entry point and the local engine stands down while one is active — never drive `react-native-google-cast` or `modules/upnp-cast` from a component. End-of-track advance and the failed-load policy (hold + notice for user moves, one auto-skip for automatic advances) are shared in `services/playback/remoteAdvance.ts`; `services/playback/streamUri.ts` / `modules/upnp-cast/.../StreamUri.kt` match stream URLs by track id, not text (auth salts differ per URL). The UPnP native module (`modules/upnp-cast`) serialises every SOAP exchange per session and stamps each poll with the load `generation` it describes; `services/upnp.ts` drops reports from earlier generations, which is what keeps a renderer's "old track stopped" from being read as the new one ending. Discovery streams `"device"` events (M-SEARCH on each Wi-Fi/Ethernet `Network`, plus `ssdp:alive` listening under a MulticastLock while the output sheet is open) and probes remembered renderers (`stores/upnp.ts` `seen`) directly. `scripts/mock-upnp-renderer.py --help` lists the real-renderer quirks each fix targets.
...
- `services/carAuto/session.ts` wires all of the above (tree pushes, play/transport listeners, now-playing + queue mirroring) and is started from `index.js`, **not** from a React component. Android Auto binds the media service without ever starting an Activity, so the JS runtime can boot with no UI at all — anything wired from a component would never run in that cold case and taps in the car would silently do nothing (issue #144). For the same reason `session.ts` calls `configurePlayback()` itself. Keep car wiring out of the React tree.
- Car work only runs **while a car host is connected**. This covers the browse-tree build, its cover mirror and the 1 s position pulse.
  - `WavioCarBrowserService` sets `CarAutoModule.carConnected` on a car host's `onConnect` and clears it in `onDestroy`. JS reads it through `CarAutoBridge.isCarConnected()` / `onCarConnection`.
  - RN pauses JS timers whenever no Activity is resumed, which is the normal state in a car. While a car is connected, `CarTimerHold.kt` holds a headless JS task (RN's supported fix). JS registers it as `WavioCarSession` in `session.ts`, and it must be registered before `notifyReady()`.
  - The task is not enough on MIUI: timers stayed frozen with it held and audio playing (measured). So car-session waits use `CarAutoBridge.delay(ms)` (= `backgroundSleep`, see **Background timers** under Playback). Never use a bare `setTimeout` / `setInterval` in car code.
  - Debug a release build's car path with `adb shell setprop log.tag.CarAuto D`.
...
- The tree is pushed whole, except album tracklists under an artist: those are listed but never prefetched (an artist such as Various Artists can own thousands). When the car opens an `album:` parent the tree lacks, `onGetChildren` waits (`PendingChildren.kt`, 8 s cap) on a `childrenRequest` that `tree.ts` `loadOnDemandChildren` answers through `CarAutoBridge.setChildren`. Opened albums are kept per auth scope and carried into every later build, so a rebuild never blanks an open album screen.
