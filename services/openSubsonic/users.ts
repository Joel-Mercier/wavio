...
export const getUser = async (username: string) => {
  try {
    const rsp = await openSubsonicApiInstance.get<OpenSubsonicResponse<{ user: User }>>(
      "/rest/getUser",
      {
        params: {
          username
...
export const getUsers = async () => {
  try {
    const rsp = await openSubsonicApiInstance.get<OpenSubsonicResponse<{ users: Users }>>(
      "/rest/getUsers",
      {
        params: {
        }
