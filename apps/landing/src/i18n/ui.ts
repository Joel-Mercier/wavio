...
export const ui = {
  ...
  ru: {
    meta: {
      ...
      homeDescription:
        'Android-плеер с открытым исходным кодом для Navidrome, Jellyfin, OpenSubsonic или твоей локальной медиатеки. Без рекламы, без слежки, работает полностью офлайн.',
    ...
    highlights: [
      ...
      'Navidrome, Jellyfin, OpenSubsonic и локальная медиатека',
    ],
    features: {
      ...
      servers: {
        ...
        desc: "Navidrome дома, Jellyfin у друга, OpenSubsonic где-то еще и локальная медиатека для подстраховки. Одно приложение для всего.",
      },
      sleep: {
        label: 'Таймер сна',
      ...
      podcasts: {
        ...
        desc: 'Поиск, подписки и воспроизведение подкастов через Taddy. Вся музыка и подкасты — в одном месте.',
      },
      radio: {
      ...
      lyrics: {
        ...
        desc: 'Тексты песен из LRCLib в режиме караоке: листаются автоматически вслед за музыкой.',
      },
      widgets: {
        label: 'Виджеты',
        title: 'Все под рукой.',
        desc: 'Управляй музыкой прямо с главного экрана. Текущий трек, недавние треки — всегда под рукой. Не нужно лишний раз открывать приложение',
      },
    },
    showcase: {
      headingPre: 'Всё продумано',
      headingEm: 'до мелочей.',
    ...
    final: {
      ...
      titleEm: 'домой.',
      desc: 'Бесплатно, с открытым исходным кодом. Сделано людьми, которые считают, что твоя медиатека должна принадлежать тебе.',
    },
    footer: {
      tagline:
        'Музыкальный Android-плеер с открытым кодом. Совместим с Navidrome, Jellyfin, OpenSubsonic и локальными медиатеками.',
      product: 'Продукт',
      features: 'Возможности',
      download: 'Скачать',
    ...
    privacy: {
      ...
      sections: [
        ...
        },
        {
          ...
          body: `<p><strong>Ничего персонального.</strong> В Wavio нет аккаунтов, профилей, рекламных SDK, поведенческой аналитики или трекеров. Мы не управляем никаким сервером, который хранит информацию о том, кто ты и что ты слушаешь.</p>
            <p>Единственные данные, которые покидают телефон, — это те, которые нужны для воспроизведения музыки или чтобы мы могли диагностировать ошибки (об этом в следующих разделах).</p>`,
        },
        {
          num: '03',
          ...
          <ul>
            <li>Данные подключения к серверам (URL, имя пользователя и зашифрованный пароль/токен).</li>
        ...
        },
        {
          ...
          <ul>
            ...
            <li><strong>Чтение медиа и хранилища</strong> чтобы скачивать сохраненные треки для офлайн-воспроизведения.</li>
              <li><strong>Вибрация</strong> для легкой виброотдачи при нажатии на некоторые кнопки.</li>
            </ul>
        ...
        },
        {
          ...
          body: `<p>Помимо твоего музыкального сервера и Sentry, Wavio общается с парой сторонних сервисов, но только если ты пользуешься соответствующей функцией:</p>
            <ul>
              ...
              <li><strong><a href="https://www.radio-browser.info/" target="_blank" rel="noopener" class="text-accent hover:underline">Radio Browser</a></strong> — предоставляет каталог интернет-радио, используется, когда ты ищешь радиостанции.</li>
              <li><strong>Google Cast</strong> — используется для трансляции на Chromecast-устройства. Имей в виду: в этом случае начинают действовать правила Google.</li>
            </ul>
            <p>В Wavio нет рекламы, аналитики или трекеров.</p>`,
        },
        {
          num: '08',
          title: 'Пользователи младшего возраста',
          body: `<p>Wavio не собирает персональные данные своих пользователей. Это правило распространяется на всех, включая детей</p>`,
        },
        {
          num: '09',
