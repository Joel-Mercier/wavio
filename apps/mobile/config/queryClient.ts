...
const mmkvQueryPersisterStorage = {
  ...
  setItem: (key: string, value: string): Promise<void> => {
    // A restore is preceded by `queryClient.resetQueries()`, whose cache events
    // reach the throttled persister *after* the scope has already flipped —
    // writing the emptied cache into the incoming scope's blob, which restore
    // is about to read. Nothing worth persisting exists until the restore lands.
