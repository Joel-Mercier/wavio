...
import { getPersistedCacheSize } from "@/config/queryPersister";
...
// Nothing to subscribe to for these, so they're a snapshot taken per mount: the
// parent remounts the overview (via `key`) after clearing the cache. Reading them
// in render instead doesn't refresh either — React Compiler treats a value that
// depends on no prop or state as constant and caches whatever uses it.
function readDiskAndCache() {
  return {
    total: Paths.totalDiskSpace || 0,
    available: Paths.availableDiskSpace || 0,
    cacheBytes: getPersistedCacheSize(),
  };
}
...
export default function StorageOverview() {
  ...
  const [{ total, available, cacheBytes }] = useState(readDiskAndCache);
  ...
  const otherBytes = Math.max(
    0,
    total - available - (firstSegment?.bytes ?? 0) - cacheBytes - prefetchBytes,
  );
