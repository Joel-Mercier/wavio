...
// The review and unmatched queues can each grow to hundreds of albums. Keeping
// both under the setup form in one scroll means reaching the second queue
// requires scrolling past the whole first one, so each lives in its own tab.
type MusicBrainzTab = "setup" | "review" | "unmatched";
...
export default function MusicBrainzScreen() {
  ...
  const [activeTab, setActiveTab] = useState<MusicBrainzTab>("setup");
  ...
  // The setup tab is always present; the queue tabs only appear once their
  // queue has something in it, mirroring the label with a live count.
  const tabs = useMemo<TabBarItem[]>(() => {
    const out: TabBarItem[] = [
      {
        key: "setup",
        title: t("app.settings.integrations.musicbrainz.tabs.setup"),
      },
    ];
    if (pending.length > 0) {
      out.push({
    ...
    if (unmatched.length > 0) {
      out.push({
        key: "unmatched",
        title: t("app.settings.integrations.musicbrainz.unmatched.title", {
          count: unmatched.length,
        }),
      });
    }
    return out;
  }, [pending.length, unmatched.length, t]);

  // Fall back to setup when the active queue empties (reset, or the last album
  // was applied/dismissed on the detail screen), so we never sit on a gone tab.
  useEffect(() => {
    if (!tabs.some((tab) => tab.key === activeTab)) setActiveTab("setup");
  }, [tabs, activeTab]);
  ...
  const handleTabPress = useCallback(
    (index: number) => {
      ...
      if (tab) setActiveTab(tab.key as MusicBrainzTab);
    },
    [tabs],
  );
  ...
  return (
    <Box className="h-full">
      <Box className={cn("pb-6 flex-1", isWideLayout ? "mb-6" : "mt-6")}>
        <HStack
          className="items-center justify-between mb-6 px-6"
        ...
        <Box className="px-6 flex-1">
          {activeTab === "review" ? (
            ...
            <FlashList
              ...
              renderItem={({ item }) => (
                <MusicBrainzUnmatchedItem album={item} />
              )}
            />
          ) : (
            <ScrollView
            ...
            >
              <VStack className="gap-y-2">
                ...
                <SettingsSectionTitle
                  title={t(
                    "app.settings.integrations.musicbrainz.fields.title",
                  )}
                />
                <Text className="text-primary-100 text-sm pb-2">
                  {t(
                    "app.settings.integrations.musicbrainz.fields.description",
                  )}
                </Text>
                ...
                {isScanning && (
                  <VStack className="gap-y-2 py-2">
                    ...
                    >
                      {t(
                        "app.settings.integrations.musicbrainz.scan.progress",
                        {
                ...
                {SUMMARY_KEY[status.phase] && (
                  <VStack className="gap-y-1 py-2">
                    ...
                    {/* The raw message, kept out of the translated sentence:
                        it's the only clue as to *why* a scan died, and it isn't
                        translatable. */}
