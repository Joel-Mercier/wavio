...
export default function AudioMuseTokenHelpDialog({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
      ...
      <AlertDialogContent className="bg-primary-800 border-primary-400">
        <AlertDialogHeader>
          <Heading className="text-white font-bold" size="md">
            {t("app.settings.integrations.audiomuse.tokenHelpTitle")}
          </Heading>
        </AlertDialogHeader>
            ...
            {STEP_KEYS.map((key, index) => (
              <HStack key={key} className="gap-x-3">
                ...
                <Text className="text-primary-50 flex-1" size="sm">
                  {t(`app.settings.integrations.audiomuse.${key}`)}
                </Text>
              </HStack>
            ))}
          </VStack>
        ...
        <AlertDialogFooter className="items-center justify-center">
          ...
          >
            <Text className="text-primary-800 font-bold text-lg">
              {t("app.settings.integrations.audiomuse.tokenHelpClose")}
            </Text>
          </FadeOutScaleDown>
        </AlertDialogFooter>
      </AlertDialogContent>
