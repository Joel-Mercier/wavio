...
// Authenticates with Subsonic auth params (token+salt, or `p=enc:` for the
// legacy mechanism), which is what the plain-password override switches
// between. An allowlist rather than a negation: Jellyfin uses an access token,
// the index-backed types have no Subsonic endpoint at all, and a backend added
