...
import { hydratePlaybackStores } from "@/services/startupHydration";
...
//
// This deliberately lives outside React, for the same reason
// services/carAuto/session.ts does: a React component's lifetime is the mounted
// surface, not the process. Android Auto boots the JS runtime with no Activity,
// so a component would never run while the car plays — and swiping the app away
// with music still going would unmount it. Both leave the watch showing its
// retained DataItems: the wrong track, with buttons that look live. Started from
// index.js so it covers every JS boot, headless or not.
//
// A watch tap still can't wake a fully closed app — see WavioWearListenerService.
//
...
// Commands held while the session finishes wiring. Hydration is short, so a
// watch that kept tapping through it meant its last taps, not its first.
const MAX_PARKED_COMMANDS = 8;

// Pairs with WearLog on the native side: a headless session has no UI to report
// from, so logcat is the only way to tell "did nothing" apart from "chose to do
// nothing".
const log = (message: string, error?: unknown) => {
  if (!__DEV__) return;
  if (error !== undefined) console.log(`[wear] ${message}`, error);
  else console.log(`[wear] ${message}`);
};

let started = false;

export function startWearSession() {
  if (started) return;
  started = true;
  void boot();
}
...
async function wire() {
  ...
  // Native sets WearBridgeModule.instance in OnCreate, before the bundle has
  // finished evaluating, so a tap landing in the boot window takes the
  // listener service's "JS is alive" path: it is neither handled here nor
  // reported as dropped, and the watch sees a successful send with nothing
  // happening. Park those until the handlers below exist — the car session
  // parks a cold tap the same way. (D6 in wear/TESTING.md.)
  let handleCommand: ((command: CommandPayload) => void) | null = null;
  const parked: CommandPayload[] = [];
  WearBridge.onCommand((command) => {
    if (handleCommand) {
      handleCommand(command);
      return;
    }
    parked.push(command);
    if (parked.length > MAX_PARKED_COMMANDS) parked.shift();
  });

  // A headless boot renders nothing under app/, so the queue still holds its
  // defaults (its persistence is hand-rolled and scoped). Publishing before it
  // is restored would overwrite the watch's retained state — which is all it has
  // to show — with an empty queue and no track. Memoized, so the car session's
  // call and this one are the same work.
  await hydratePlaybackStores().catch((e) => log("hydration failed", e));
  ...
  WearBridge.onConnection((connected) => {
    hasWatch = connected;
    if (!connected) {
      subscribedUntil = 0;
      stopProgressTimer();
  ...
  useAuthBase.subscribe(() => {
    ...
    // above turns into a push of the incoming scope's state.
  });
  ...
  handleCommand = (command) => {
    switch (command.action) {
      ...
      case "unsubscribe":
        ...
        stopProgressTimer();
        break;
      // The phone's capability lookup missed this watch, so `hasWatch` may be
      // flipping here for the first time — and MainActivity fires `hello` and
      // `subscribe` from independent coroutines, so a `subscribe` already
      // handled while it was false started no ticker.
      case "hello":
        hasWatch = true;
        resync();
        syncProgressTimer();
  ...
  for (const command of parked.splice(0)) handleCommand(command);
}
