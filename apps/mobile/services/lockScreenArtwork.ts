...
// Local mirror of the now-playing cover, so the OS media controls never have to
// fetch it themselves.
//
// expo-audio loads `artworkUrl` natively and cannot authenticate: Android does a
// bare `url.openConnection().getInputStream()` (AudioControlsService.kt) and iOS
// a bare `URLSession.shared.dataTask` (MediaController.swift). Neither carries
// the server's custom headers, its client certificate, or the trust decision for
// a self-signed certificate — and both swallow the failure, so the notification
// silently shows no artwork. Handing them a `file://` URI sidesteps the native
// fetch entirely; both platforms read a local file through the same code path.
//
...
const artworkDir = (): Directory =>
  new Directory(Paths.cache, "lockscreen-artwork");
...
// No extension: both decoders sniff the content (BitmapFactory, UIImage), and a
// cover served as PNG under a .jpg name would be a lie on disk.
...
function isFresh(file: File): boolean {
  ...
  // Missing timestamp (not reported on every platform) — treat as fresh and let
  // the entry age out via the count cap instead.
 ...
 *
 * Synchronous by design: the caller applies lock-screen metadata during a track
 * change and needs an answer without yielding, so a warm cover appears on the
 * very first notification rather than flashing in a moment later.
 */
export function cachedArtworkUri(
...
// Drop the oldest entries once the directory outgrows the cap, and anything past
// its TTL. Runs after a successful download, so the cap is enforced lazily
 ...
 *
 * Returns undefined when the URL isn't remote or the download failed — callers
 * fall back to handing the OS the remote URL, which is no worse than before.
 */
export async function ensureArtworkCached(
  ...
  const task = (async () => {
    try {
      ...
      // A stale-but-present file would make downloadFileAsync a no-op or a
      // conflict; clear it so the refresh actually refetches.
      if (target.exists) {
        try {
          target.delete();
        } catch {}
      ...
      const result = await File.downloadFileAsync(remoteUrl, target, {
        idempotent: true,
      ...
      if (!looksLikeImage((await result.bytes()).subarray(0, 4))) {
      ...
      // Not reported to Sentry: a server that's briefly unreachable, or a track
      // with no cover, is an ordinary outcome. The notification just keeps
...
/**
 * Drop every mirrored cover. Registered as a logout handler in
 * services/player.ts — the entries belong to a server the user is leaving, and
 * the whole directory is re-derivable, so there's nothing to preserve.
 */
