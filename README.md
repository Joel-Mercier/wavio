...
5. Build and install the watch app **to an explicit serial**. It shares the phone's `applicationId`, so a bare `./gradlew :wear:installDebug` with both devices attached will replace Wavio on the phone:
   ...
   adb -s <watch-serial> shell am start -n com.jmercier.wavio/com.jmercier.wavio.wear.MainActivity
   ```
6. Both sides log under the same tag: `adb -s <phone-serial> logcat -s WavioWear ReactNativeJS` and `adb -s <watch-serial> logcat -s WavioWear`. The phone's JS layer logs under `[wear]` in `ReactNativeJS`, and only in `__DEV__`.
